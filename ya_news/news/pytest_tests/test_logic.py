...
def test_reader_cant_edit_authors_comment(
        comment, reader_client, news_edit_url
):
...
def test_delete_comment_by_author(author_client, news_delete_url):
...
def test_author_can_edit_comment(
        comment, news_detail_url, author_client, news_edit_url
):
    assertRedirects(
        author_client.post(news_edit_url, data=NEW_TEXT_FOR_COMMENTS),
        f'{news_detail_url}#comments'
    )
...
def test_delete_comment_by_reader(comment, reader_client, news_delete_url):
    comments_before = len(Comment.objects.all())
    reader_client.post(news_delete_url)
